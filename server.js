...
const server = http.createServer(async (req, res) => {
  ...
  if (urlParts[1] === "api" && req.method === "GET") {
    ...
    if (req.url.startsWith("/api/continent/") && urlParts[3]) {
      const destinations = data.filter(
        (item) => item.continent.toLowerCase() === urlParts[3].toLowerCase(),
      );
      if (destinations) res.write(JSON.stringify(destinations) + "\n");
      else {
        res.statusCode = 404;
